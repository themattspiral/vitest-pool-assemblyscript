...
4. **`static` method hit count** — reported **0** in AssemblyScript (entry counting — execution-accurate), but the merged V8/JS report shows a static method's count as the *class-definition* count (≈1) regardless of its real call count. The `1` is **not** from V8 execution — raw V8 reports an uncalled method as 0 — it appears to be a bug in vitest's AST-aware coverage remapper (`ast-v8-to-istanbul`, the default in v4/v5): the remapper looks up a method's hit count at the `MethodDefinition` node's start offset, which for a `static` method is the `static` keyword. V8 starts a static method's coverage range at the method *name*, so the `static` keyword sits *outside* it, and the lookup falls through to the enclosing class/module range (executed once when the class is defined). The result: an uncalled static reads 1 (should be 0) **and** a static called N times reads ~1 (should be N). Instance methods, getters, and setters are unaffected (their ranges start at/before the lookup offset). AssemblyScript's value is the execution-accurate one.
