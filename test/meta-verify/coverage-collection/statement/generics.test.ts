...
describe.runIf(COVERAGE_ENABLED)('coverage collection — generic (monomorphized) statements', () => {
  ...
  describe('AS statement hits SUM across monomorphizations', () => {
    test('clamp body: each instance runs once, summed at the single source', () => {
      expect(statementHitsByLine(as, 6)).toEqual([2]);  // if (v < lo): i32 + f64
      expect(statementHitsByLine(as, 7)).toEqual([0]);  // return lo: neither < lo
      expect(statementHitsByLine(as, 9)).toEqual([2]);  // if (v > hi): i32 + f64
      expect(statementHitsByLine(as, 10)).toEqual([1]); // return hi: f64 only (15 > 10)
      expect(statementHitsByLine(as, 12)).toEqual([1]); // return v: i32 only (5 in range)
    });

    test('callers each run once', () => {
      expect(statementHitsByLine(as, 16)).toEqual([1]); // useI32 → clamp<i32>
      expect(statementHitsByLine(as, 20)).toEqual([1]); // useF64 → clamp<f64>
    });
  });
  ...
  describe('parity with v8 (one function called twice == SUM of two monomorphizations)', () => {
    const ALIGNED = [6, 7, 9, 10, 12, 16, 20];
